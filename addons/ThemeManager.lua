...
local ThemeManager = {} do
    ...
    function ThemeManager:SaveConfig(configName)
        ...
        for _, option in next, Options do
            if option.Type ~= "Button" and option.Flag and option.Value then
                config[option.Flag] = option.Value
            end
        end

        writefile(self.Folder .. '/configs/' .. configName .. '.json', httpService:JSONEncode(config))
    end

    function ThemeManager:LoadConfig(configName)
    ...
    function ThemeManager:GetConfigList()
        ...
        for i = 1, #list do
            ...
            if file:sub(-5) == '.json' then
                ...
                if char == '/' or char == '\\' then
                    table.insert(configs, file:sub(pos + 1, -6))
                end
            end
        end
    ...
    function ThemeManager:CreateThemeManager(groupbox)
        ...
        groupbox:AddButton('Save config', function()
            ...
            self.Library:Notify(string.format('Saved config: %q', Options.ConfigName.Value))
        end)

        groupbox:AddDropdown('ConfigList', { Text = 'Config list', Values = self:GetConfigList(), AllowNull = true })

        groupbox:AddButton('Load config', function()
            if Options.ConfigList.Value then
                self:LoadConfig(Options.ConfigList.Value)
                self.Library:Notify(string.format('Loaded config: %q', Options.ConfigList.Value))
            end
        end)
        ...
        groupbox:AddButton('Refresh list', function()
            ...
            Options.ConfigList.Values = self:GetConfigList()
